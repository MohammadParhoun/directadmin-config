...
response=$(curl -s -m $API_TIMEOUT -u "admin:$adminpassword" \
-d "add=Save" \
...
"http://127.0.0.1:$da_port/CMD_API_MANAGE_USER_PACKAGES")
...
response=$(curl -s -m $API_TIMEOUT -u "admin:$adminpassword" "http://127.0.0.1:$da_port/CMD_API_ACCOUNT_USER" \
-d "action=create" \
-d "add=Submit" \
